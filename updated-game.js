class ModernSalesGame {
    ...
    generateClientGreeting(client) {
        const greetings = {
            cafe: `Здравствуйте! Я ${client.name}, открываю кафе. Нужно оборудование на ${this.formatMoney(client.budget)}.`,
            restaurant: `Добро пожаловать! ${client.name}, владелец ресторана. Ищу качественное оборудование, бюджет ${this.formatMoney(client.budget)}.`,
            hotel: `Приветствую! ${client.name} из отеля. Нам требуется профессиональное оборудование, готовы потратить ${this.formatMoney(client.budget)}.`,
            canteen: `Здравствуйте! ${client.name}, заведующий столовой. Нужно надежное оборудование, бюджет ограничен ${this.formatMoney(client.budget)}.`
        };
